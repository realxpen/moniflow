...
export default function NativeWalletSetupScreen() {
  ...
  const update = (key: string, state: StepState) =>
  ...
  const provision = async () => {
    ...
    try {
      ...
      const walletPayload = (await walletResponse.json()) as {
        wallet?: { smartWalletAddress?: string };
      };
      if (!walletResponse.ok || !walletPayload.wallet?.smartWalletAddress) {
        throw new Error("CNGN smart wallet could not be created.");
      }
      ...
      setPin("");
      setError(
        cause instanceof Error ? cause.message : "Wallet provisioning failed safely."
      );
      setStates((current) => {
    ...
    <Screen contentContainerStyle={styles.screen}>
      ...
      <BlurView intensity={38} tint="light" style={styles.panel}>
        ...
        <View style={styles.steps}>
          {steps.map((step) => (
            <ProvisionStep key={step.label} {...step} />
          ))}
        </View>
        ...
        {smartWalletAddress ? (
          <Text style={styles.address}>CNGN · {smartWalletAddress}</Text>
        ) : null}
        ...
        <Text style={styles.security}>
          Private keys never leave this device. MONIFlow sends only the owner address and
          cryptographic signatures to the backend.
        </Text>
      </BlurView>
    </Screen>
  );
...
function ProvisionStep({ label, state }: Step) {
  const symbol =
    state === "done" ? "✓" : state === "working" ? "•••" : state === "error" ? "!" : "○";
  return (
    <View style={styles.step}>
      <View
        style={[
          ...
          state === "working" && styles.dotWorking
        ]}
      />
