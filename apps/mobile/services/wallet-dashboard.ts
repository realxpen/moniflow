...
export async function loadWalletBalance(localUserId: string) {
  ...
  return { ...payload.balance, amount: payload.balance.availableToSpend };
}

export async function loadDepositAccount(localUserId: string) {
  const response = await apiFetch(`${resolveApiUrl()}/api/wallet/deposit-account?localUserId=${encodeURIComponent(localUserId)}`);
