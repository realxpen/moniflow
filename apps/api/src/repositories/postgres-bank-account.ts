...
const rowSchema = z.object({
  ...
  bank_name: z.string(), masked_account_number: z.string(), account_holder_name: z.string(), verified: z.boolean(),
...
export async function ensureBankAccountSchema(sql: Sql) {
  ...
  await sql`update moniflow_private.bank_accounts set label = coalesce(label, bank_name), normalized_label = coalesce(normalized_label, lower(regexp_replace(coalesce(label, bank_name), '[^a-zA-Z0-9]', '', 'g'))) where label is null or normalized_label is null`;
...
export class PostgresBankAccountRepository implements BankAccountRepository {
  ...
  async findVerifiedByLabel(localUserId: string, label: string): Promise<VerifiedBankAccount | null> {
    const rows = await this.sql`
      select id, local_user_id, label, provider_account_id, bank_code, bank_name, masked_account_number,
             account_holder_name, verified, created_at, updated_at
      from moniflow_private.bank_accounts
      where local_user_id = ${localUserId}::uuid and normalized_label = ${normalizeBankLabel(label)} and verified = true
      limit 1
    `;
  ...
  async findVerifiedByProviderId(localUserId: string, providerAccountId: string): Promise<VerifiedBankAccount | null> {
    const rows = await this.sql`
      select id, local_user_id, label, provider_account_id, bank_code, bank_name, masked_account_number,
             account_holder_name, verified, created_at, updated_at
      from moniflow_private.bank_accounts
      where local_user_id = ${localUserId}::uuid and provider_account_id = ${providerAccountId} and verified = true
      limit 1
    `;
    return rows[0] ? mapRow(rowSchema.parse(rows[0])) : null;
  }
  ...
  async saveVerified(account: VerifiedBankAccount): Promise<VerifiedBankAccount> {
    await this.sql`
      insert into moniflow_private.bank_accounts (
        id, local_user_id, label, normalized_label, provider_account_id, bank_code, bank_name,
        masked_account_number, account_holder_name, verified, created_at, updated_at
      ) values (
        ${account.id}::uuid, ${account.localUserId}::uuid, ${account.label}, ${normalizeBankLabel(account.label)},
        ${account.providerAccountId}, ${account.bankCode}, ${account.bankName}, ${account.maskedAccountNumber},
        ${account.accountHolderName}, true, ${account.createdAt}::timestamptz, ${account.updatedAt}::timestamptz
      )
...
function mapRow(row: z.infer<typeof rowSchema>): VerifiedBankAccount {
  return {
    ...
    accountHolderName: row.account_holder_name, verified: true,
