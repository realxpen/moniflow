...
export const walletRoutes: FastifyPluginAsync<WalletRouteOptions> = async (app, options) => {
  app.get<{ Querystring: { localUserId?: string } }>("/", async (request, reply) => {
    const context = await resolveContext(
      request.query.localUserId,
      options.getBmoniUserService(),
      options.getWalletOwnershipRepository(),
      reply
    );
    if (!context) return;
    try {
      ...
      const record = findRecord(payload, (candidate) =>
        stringValue(candidate, ["id", "smartWalletId", "walletId", "groupWalletId"]) === context.wallet.bmoniSmartWalletId ||
        stringValue(candidate, ["address", "walletAddress", "smartAccountAddress", "safeAddress"])?.toLowerCase() === context.wallet.smartWalletAddress.toLowerCase()
      ) ?? asRecord(payload);
      ...
      return {
        wallet: {
          id: stringValue(record, ["smartWalletId", "walletId", "groupWalletId", "id"]) ?? context.wallet.bmoniSmartWalletId,
          address: stringValue(record, ["address", "walletAddress", "smartAccountAddress", "safeAddress"]) ?? context.wallet.smartWalletAddress,
          currency: stringValue(record, ["currency", "symbol"]) ?? "CNGN",
          status: normalizeWalletStatus(stringValue(record, ["status", "state"]), record.isActive)
        }
      };
    } catch (error) {
  ...
  app.get<{ Querystring: { localUserId?: string } }>("/balance", async (request, reply) => {
    ...
    try {
      ...
      return { balance: { amount: balance.amount, currency: "CNGN", fiatCurrency: "NGN", source: "bmoni" } };
    } catch (error) {
...
async function resolveContext(
  ...
  ownership: WalletOwnershipRepository,
  reply: FastifyReply
) {
...
function findBalance(payload: unknown, currency: string): { amount: string } | null {
  const record = findRecord(payload, (candidate) => stringValue(candidate, ["currency", "symbol", "asset", "token", "code"])?.toUpperCase() === currency);
  if (!record) return null;
  const amount = stringValue(record, ["availableBalance", "available", "balance", "amount", "total"]);
  return amount === undefined ? null : { amount };
}
function normalizeWalletStatus(status: string | undefined, isActive: unknown) {
  if (!status) return isActive === true ? "active" : isActive === false ? "processing" : "unknown";
  const value = status.toLowerCase();
  if (["active", "ready", "enabled", "completed"].includes(value)) return "active";
  if (["pending", "processing", "provisioning", "created", "preparing"].includes(value)) return "processing";
...
function handleBmoniError(app: FastifyInstance, reply: FastifyReply, error: unknown, operation: string) {
  ...
  if (error instanceof BmoniProviderError) {
    ...
    const statusCode = [400, 404, 409].includes(error.statusCode) ? error.statusCode : 502;
