...
export const walletRoutes: FastifyPluginAsync<WalletRouteOptions> = async (app, options) => {
  ...
  app.get<{ Querystring: { localUserId?: string } }>("/", async (request, reply) => {
    const context = await resolveContext(request.query.localUserId, options.getBmoniUserService(), ownership, reply);
    if (!context) return;
    try {
    ...
    } catch (error) { return handleBmoniError(app, reply, error, "wallet lookup"); }
  });
  ...
  app.get<{ Querystring: { localUserId?: string } }>("/balance", async (request, reply) => {
    ...
    } catch (error) { return handleBmoniError(app, reply, error, "balance lookup"); }
  });
...
async function resolveContext(rawLocalUserId: string | undefined, userService: BmoniUserService, ownership: WalletOwnershipRepository, reply: FastifyReply) {
  ...
  if (!mapping) { reply.status(409).send({ statusCode: 409, error: "Conflict", message: "Create the BMONI user before loading wallet information." }); return null; }
...
function findRecord(value: unknown, predicate: (record: JsonRecord) => boolean): JsonRecord | null {
  ...
  const record = asRecord(value); if (!record) return null; if (predicate(record)) return record;
...
function handleBmoniError(app: FastifyInstance, reply: FastifyReply, error: unknown, operation: string) {
  ...
  if (error instanceof BmoniProviderError) { app.log.warn({ errorName: error.name, requestId: error.requestId, statusCode: error.statusCode }, `BMONI ${operation} failed`); const statusCode = [400,404,409].includes(error.statusCode) ? error.statusCode : 502; return reply.status(statusCode).send({ statusCode, error: "Upstream Error", message: `BMONI rejected the ${operation}.` }); }
