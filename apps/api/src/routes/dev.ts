...
export const devRoutes: FastifyPluginAsync<DevRouteOptions> = async (app, options) => {
  ...
  app.get<{ Querystring: unknown }>("/bmoni-status", async (request, reply) => {
    ...
    try {
      ...
      const mapping = query.data.localUserId ? await options.getBmoniUserService().getMapping(query.data.localUserId) : null;
      return reply.send({
        bmoniApi: "connected",
        environment,
  ...
  app.get<{ Querystring: unknown }>("/bmoni-lifecycle", async (request, reply) => {
    ...
    if (!query.success) return reply.status(400).send({ statusCode: 400, error: "Bad Request", message: "localUserId must be a valid UUID." });
    ...
    try {
      ...
      if (!mapping || !wallet) return reply.send({ environment, localUserId, readyForPhase11: false, stages });
      ...
      stages.nigeriaRail = { passed: railReady, detail: railReady ? "BMONI reports Nigeria/NGN active" : "Nigeria/NGN not confirmed active" };
      ...
      try {
        ...
        stages.depositAccount = { passed: Boolean(depositRecord), detail: depositRecord ? "NGN deposit account returned by BMONI" : "No NGN deposit account found" };
      } catch (error) {
        if (error instanceof BmoniProviderError && error.statusCode === 404) stages.depositAccount = { passed: false, detail: "NGN deposit account not available yet" };
        else throw error;
      }

      const cngn = findCngnBalance(await gateway.listAccountBalances(mapping.bmoniUserId));
      ...
      const readyForPhase11 = stages.api.passed && stages.user.passed && stages.wallet.passed && stages.nigeriaRail.passed && stages.fundedBalance.passed;
      ...
      if (isBmoniError(error)) {
        ...
        return reply.status(503).send({ environment, localUserId, readyForPhase11: false, stages, error: "BMONI lifecycle verification could not complete." });
      }
      throw error;
    }
  });
...
function isBmoniError(error: unknown): error is BmoniConfigurationError | BmoniProviderError | BmoniResponseValidationError | BmoniTransportError {
  return error instanceof BmoniConfigurationError || error instanceof BmoniTransportError || error instanceof BmoniProviderError || error instanceof BmoniResponseValidationError;
}
function findCngnBalance(payload: unknown): string | null {
  const record = findRecord(payload, (candidate) => stringValue(candidate, ["currency", "symbol", "asset", "token", "code"])?.toUpperCase() === "CNGN");
